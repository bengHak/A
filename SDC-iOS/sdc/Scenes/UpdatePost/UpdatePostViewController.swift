...
class UpdatePostViewController: UIViewController {
    ...
    // MARK: - Properties
    var bag = DisposeBag()
    var viewModel = UpdatePostViewModel()
    ...
    // MARK: - Lifecycle
    init(postId: Int, title: String, content: String) {
        self.postId = postId
    ...
    private func bindButtons() {
        uploadButton.rx.tap
            .subscribe(onNext: { [weak self] in
                ...
                self.viewModel.uploadPostContent(postId: postId, title: title, content: content)
            }).disposed(by: bag)
